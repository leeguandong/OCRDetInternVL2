...
CACHE_URL_BASE = 'https://hf-mirror.com/pharmapsychotic/ci-preprocess/resolve/main/'
...
class Config:
    ...
    data_path = "/home/lgd/e_commerce_lmm/scripts/data/"
...
def save_to_jsonl(save_file, image_path):
    st = time.time()
        ...
        for root, dirs, files in os.walk(image_path):
            for file in files:
                ...
                try:
                    ...
                    ocr_text = ci.interrogate_fast(image)
                    print(ocr_text)
                    data = {
                        "text": ocr_text,
                        "image": os.path.join("images",file),
                        "conditioning_image": os.path.join("condition_images", file)  # 根据实际情况修改
                    }
...
if __name__ == "__main__":
    ...
    image_path = "/home/lgd/e_commerce_sd/data/mask_controlnet/images/"
    # image_mask_path = "/home/lgd/e_commerce_sd/data/mask_controlnet/condition_images/"
    save_file_ = f"image_ocr_{datetime.now().strftime('%Y%m%d%H%M%S')}.jsonl"
    # image = Image.open(image_path).convert('RGB')
    ci = Interrogator(Config(clip_model_name="ViT-L-14/openai"))
    # print(ci.interrogate(image))
    save_to_jsonl(save_file_, image_path)
